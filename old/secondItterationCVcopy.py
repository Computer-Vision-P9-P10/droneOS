...
# This is a version the one used from P9 on the jetson orin nano itself
class YOLOInferenceLogger:
    ...
    def run_inference(self):
        ...
        while self.cap.isOpened():
            ...
            class_ids = (
                results[0].boxes.cls.cpu().numpy().tolist() if num_dets > 0 else []
            )
            ...
            if self.frame_count % self.log_interval == 0:
                ...
                inf_fps = (
                    len(self.inference_times) / sum(self.inference_times)
                    if self.inference_times
                    else 0
                )
                # This FPS is "How fast the CPU+GPU processes a frame ONCE DECODED"
                pipe_fps = (
                    len(self.pipeline_times) / sum(self.pipeline_times)
                ...
                print(
                    f"Frame {self.frame_count}: Inference FPS={inf_fps:.1f}, Pipeline FPS={pipe_fps:.1f}, Dets={num_dets} ({class_str})"
                )
                ...
                with open(self.csv_file, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(
                        [self.frame_count, num_dets, class_names_list, confs]
                    )
        ...
        avg_inf_fps = (
            self.frame_count / self.total_inference_time
        ...
        print(
            f"Real World FPS:    {real_world_fps:.1f} (Includes 4K Decoding + Resize + Inference)"
        )
        print("-" * 30)
        ...
        for entry in self.detections_log[-3:]:
            class_str = (
                ", ".join(entry["class_names"]) if entry["class_names"] else "None"
            )
